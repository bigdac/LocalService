...
public class HelpService extends Service {
    ...
    public void onCreate() {
        ...
        if (Build.VERSION.SDK_INT>=Build.VERSION_CODES.O){
            NotificationChannel channel = new NotificationChannel("deamo","deamo", NotificationManager.IMPORTANCE_LOW);
            NotificationManager manager = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
            if (manager==null)
                return;
            manager.createNotificationChannel(channel);
            Notification notification = new NotificationCompat.Builder(this,
                    "deamo").setAutoCancel(true).setCategory(
    ...
    private class MyServiceConnection implements ServiceConnection {
        ...
        public void onServiceDisconnected(ComponentName name) {
            startService(new Intent(HelpService.this,LocalService.class));
            bindService(new Intent(HelpService.this,LocalService.class),new MyServiceConnection(),BIND_AUTO_CREATE);
        }
    }
}
